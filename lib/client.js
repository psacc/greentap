/**
 * greentap client — connects to the daemon via CDP, or auto-starts one.
 *
 * Usage:
 *   const { page, disconnect } = await connect();
 *   // ... use page ...
 *   await disconnect();
 */

import { chromium } from "playwright";
import { isIP } from "net";
import { lookup } from "dns/promises";
import { join, dirname } from "path";
import { homedir } from "os";
import { fork } from "child_process";
import { fileURLToPath } from "url";
import {
  readFileSync,
  writeFileSync,
  unlinkSync,
  readdirSync,
  utimesSync,
} from "fs";
import { detectLocale } from "./locale.js";

// GREENTAP_DIR honors an env override (matches lib/daemon.js) so tests can
// isolate from the user's real ~/.greentap. Production never sets it.
const GREENTAP_DIR = process.env.GREENTAP_DIR || join(homedir(), ".greentap");
const PORT_FILE = join(GREENTAP_DIR, "daemon.port");
const PID_FILE = join(GREENTAP_DIR, "daemon.pid");
const LOCK_FILE = join(GREENTAP_DIR, "daemon.lock");
const HEARTBEAT_FILE = join(GREENTAP_DIR, "daemon.heartbeat");
const BROWSER_DATA_DIR = join(GREENTAP_DIR, "browser-data");
const WA_URL = "https://web.whatsapp.com";
// GREENTAP_CDP_URL points at a daemon this process does NOT own — one browser,
// one linked device, several callers that cannot each launch their own. On one
// host that is what the port file already gives you; across container
// boundaries there is no shared filesystem and no shared network namespace, so
// the endpoint has to be named explicitly. When it is set, this module never
// launches or stops anything: it is a client of somebody else's browser.
const REMOTE_CDP_URL = process.env.GREENTAP_CDP_URL || null;

function touchHeartbeat() {
  // Daemon polls HEARTBEAT_FILE mtime to detect activity. Touching it on each
  // successful connect resets the daemon's idle countdown — replaces the
  // CDP-event hook in lib/daemon.js that never fired for connectOverCDP.
  try {
    const now = new Date();
    utimesSync(HEARTBEAT_FILE, now, now);
  } catch {
    try {
      writeFileSync(HEARTBEAT_FILE, "", { mode: 0o600 });
    } catch {
      // best effort — daemon will recreate on next poll if missing
    }
  }
}

const __dirname = dirname(fileURLToPath(import.meta.url));
const DAEMON_SCRIPT = join(__dirname, "daemon.js");

class BlockingModalError extends Error {
  constructor() {
    super(
      "Unknown blocking WhatsApp modal detected. " +
        "Run `greentap daemon stop`, then `greentap login`; " +
        "inspect and dismiss the dialog manually.",
    );
    this.name = "BlockingModalError";
  }
}

function readPort() {
  try {
    return parseInt(readFileSync(PORT_FILE, "utf8").trim(), 10);
  } catch {
    return null;
  }
}

function readPid() {
  try {
    const pid = parseInt(readFileSync(PID_FILE, "utf8").trim(), 10);
    // Reject non-positive / NaN pids. A corrupted pid file containing "-1"
    // would otherwise flow into process.kill(-1, ...) — a process-GROUP
    // signal that fans out across the user's processes. greentap only ever
    // writes a real positive child pid, so anything else is corruption.
    return Number.isInteger(pid) && pid > 0 ? pid : null;
  } catch {
    return null;
  }
}

function hasSession() {
  try {
    const entries = readdirSync(BROWSER_DATA_DIR);
    return entries.length > 0;
  } catch {
    return false;
  }
}

function cleanupStaleFiles() {
  for (const f of [PORT_FILE, PID_FILE, LOCK_FILE]) {
    try {
      unlinkSync(f);
    } catch {
      // already gone
    }
  }
}

function isProcessAlive(pid) {
  // Defense in depth: a non-positive pid makes process.kill signal a process
  // GROUP (or every process, for -1). Never probe/​signal those.
  if (!Number.isInteger(pid) || pid <= 0) return false;
  try {
    process.kill(pid, 0);
    return true;
  } catch {
    return false;
  }
}

/**
 * Acquire a lockfile. Uses O_EXCL for atomic creation + PID for staleness detection.
 * If the lock is held by a dead process, it's cleaned up and re-acquired.
 */
function acquireLockSync() {
  // Write our PID to the lockfile atomically
  try {
    writeFileSync(LOCK_FILE, String(process.pid), { flag: "wx", mode: 0o600 });
    return true;
  } catch (err) {
    if (err.code !== "EEXIST") throw err;
  }

  // Lock exists — check if holder is alive
  try {
    const holderPid = parseInt(readFileSync(LOCK_FILE, "utf8").trim(), 10);
    if (holderPid && isProcessAlive(holderPid)) {
      return false; // lock is legitimately held
    }
  } catch {
    // can't read lock — try to take it
  }

  // Stale lock — remove and retry
  try {
    unlinkSync(LOCK_FILE);
  } catch {
    // race condition, another process may have removed it
  }
  try {
    writeFileSync(LOCK_FILE, String(process.pid), { flag: "wx", mode: 0o600 });
    return true;
  } catch {
    return false;
  }
}

function releaseLock() {
  try {
    unlinkSync(LOCK_FILE);
  } catch {
    // best effort
  }
}

/**
 * Chrome's DevTools HTTP endpoint refuses a request whose Host header is
 * neither an IP literal nor "localhost", so `http://greentap-daemon:19223` — a
 * container reached by its DNS name on a container network — is rejected before
 * any WebSocket is opened. Resolve the name here and connect by address.
 */
export async function resolveCdpUrl(raw) {
  const u = new URL(raw);
  if (isIP(u.hostname) || u.hostname === "localhost") return u.toString();
  const { address, family } = await lookup(u.hostname);
  u.hostname = family === 6 ? `[${address}]` : address;
  return u.toString();
}

async function tryConnectUrl(url) {
  const browser = await chromium.connectOverCDP(url, { timeout: 5000 });
  const context = browser.contexts()[0];
  if (!context) {
    await browser.close();
    throw new Error("No context found on daemon");
  }
  const page = context.pages()[0];
  if (!page) {
    await browser.close();
    throw new Error("No page found on daemon");
  }
  touchHeartbeat();
  return { browser, page };
}

async function tryConnect(port) {
  return tryConnectUrl(`http://127.0.0.1:${port}`);
}

async function startDaemon() {
  const child = fork(DAEMON_SCRIPT, [], {
    detached: true,
    stdio: "ignore",
  });
  child.unref();

  // Poll for port file (max 15s)
  const deadline = Date.now() + 15000;
  while (Date.now() < deadline) {
    await new Promise((r) => setTimeout(r, 300));
    const port = readPort();
    if (port) return port;
  }
  throw new Error("Daemon failed to start within 15s");
}

async function dismissKnownInformationalModal(page) {
  const dialogs = page.locator('[role="dialog"][aria-modal="true"]:visible');
  const dialogCount = await dialogs.count();
  if (dialogCount === 0) return false;
  if (dialogCount !== 1) throw new BlockingModalError();

  const dialog = dialogs.first();
  const popup = dialog.locator('[data-testid="confirm-popup"]');
  const title = popup.locator('[data-testid="popup-title"]');
  const contents = popup.locator('[data-testid="popup-contents"]');
  const continueButton = contents.locator(
    '[data-testid="whats_new_continue_button"]',
  );
  const buttons = dialog.getByRole("button");

  const isKnownInformationalModal =
    (await popup.count()) === 1 &&
    (await popup.getAttribute("data-animate-modal-body")) === "true" &&
    (await title.locator('svg[viewBox="0 0 88 88"]').count()) === 1 &&
    (await title.getByRole("button").count()) === 1 &&
    (await contents.locator('svg[viewBox="0 0 24 24"]').count()) === 4 &&
    (await contents.getByRole("button").count()) === 1 &&
    (await continueButton.count()) === 1 &&
    (await buttons.count()) === 2;

  if (!isKnownInformationalModal) throw new BlockingModalError();

  await continueButton.click({ timeout: 3000 });
  await dialog.waitFor({ state: "detached", timeout: 3000 });
  return true;
}

export async function ensureChatList(page) {
  await dismissKnownInformationalModal(page);
  const grid = page.getByRole("grid").first();

  // Quick check
  try {
    await grid.waitFor({ timeout: 3000 });
    return;
  } catch {
    // not visible, try recovery
  }

  // Escape (dismiss overlays)
  await page.keyboard.press("Escape");
  await dismissKnownInformationalModal(page);
  try {
    await grid.waitFor({ timeout: 3000 });
    return;
  } catch {
    // still not visible
  }

  // Reload
  await page.reload();
  await dismissKnownInformationalModal(page);
  try {
    await grid.waitFor({ timeout: 10000 });
    return;
  } catch {
    // still not visible
  }

  // Last resort: navigate
  await page.goto(WA_URL);
  await dismissKnownInformationalModal(page);
  try {
    await grid.waitFor({ timeout: 15000 });
    return;
  } catch {
    throw new Error("WhatsApp session not responding. Run `greentap login` to re-authenticate.");
  }
}

/**
 * Connect to the daemon, auto-starting if needed.
 * Returns { page, disconnect, localeConfig }.
 */
export async function connect() {
  if (REMOTE_CDP_URL) return connectRemote(REMOTE_CDP_URL);

  // Guard: no session
  if (!hasSession()) {
    throw new Error("No session. Run `greentap login` first.");
  }

  // Try existing daemon (with CDP-listening retry for cold start).
  // Since #13 fix, the daemon writes the port file *before* Chrome is
  // listening, so the first tryConnect may fail while Chrome is still
  // warming up (15-30s). Retry while the daemon PID is alive.
  let port = readPort();
  if (port) {
    const pid = readPid();
    const daemonAlive = pid !== null && isProcessAlive(pid);
    const deadline = Date.now() + (daemonAlive ? 30000 : 5000);
    while (Date.now() < deadline) {
      try {
        const { browser, page } = await tryConnect(port);
        await ensureChatList(page);
        const localeConfig = await detectLocale(page);
        return {
          page,
          disconnect: () => browser.close(),
          localeConfig,
        };
      } catch (err) {
        if (err instanceof BlockingModalError) throw err;
        if (!daemonAlive) break;
        await new Promise((r) => setTimeout(r, 500));
      }
    }
    // Reaching here means every tryConnect within the deadline threw.
    // If the daemon PID was dead, files are safely stale — clean up and
    // fall through to spawning a fresh one.
    // If the daemon PID was alive but we never got a usable CDP connection,
    // the daemon is wedged (started but can't serve). Surface an actionable
    // error instead of silently deleting its metadata — otherwise we'd
    // spawn a second daemon that collides on port 19222.
    if (daemonAlive) {
      throw new Error(
        `Daemon (PID ${pid}) is running but not responding on CDP within 30s. ` +
          `Run \`greentap daemon stop\` (or kill -9 ${pid}) and retry.`,
      );
    }
    cleanupStaleFiles();
  }

  // Start daemon under lock
  // Poll for lock acquisition (another CLI may be starting the daemon)
  const lockDeadline = Date.now() + 20000;
  let gotLock = false;
  while (Date.now() < lockDeadline) {
    gotLock = acquireLockSync();
    if (gotLock) break;

    // Another process holds the lock — wait and check if daemon appeared
    await new Promise((r) => setTimeout(r, 500));
    port = readPort();
    if (port) {
      // Daemon started by another process
      const { browser, page } = await tryConnect(port);
      await ensureChatList(page);
      const localeConfig = await detectLocale(page);
      return { page, disconnect: () => browser.close(), localeConfig };
    }
  }

  if (!gotLock) {
    throw new Error("Could not acquire daemon lock within 20s");
  }

  try {
    // Double-check: daemon may have started while we waited
    port = readPort();
    if (!port) {
      port = await startDaemon();
    }
  } finally {
    releaseLock();
  }

  const { browser, page } = await tryConnect(port);
  await ensureChatList(page);
  const localeConfig = await detectLocale(page);
  return {
    page,
    disconnect: () => browser.close(),
    localeConfig,
  };
}

/**
 * Connect to a daemon owned by somebody else (GREENTAP_CDP_URL).
 *
 * Deliberately does none of the local path's lifecycle work. The profile is not
 * on this filesystem, so `hasSession()` cannot answer; there is no port file,
 * no PID to probe and no lock to take, and spawning a second daemon would be a
 * second browser on a profile Chromium locks to one process. The retry window
 * matches the local path's, because the remote daemon may be cold-starting for
 * the same 15-30s reason.
 */
async function connectRemote(rawUrl) {
  const url = await resolveCdpUrl(rawUrl);
  const deadline = Date.now() + 30000;
  let last;
  for (;;) {
    try {
      const { browser, page } = await tryConnectUrl(url);
      await ensureChatList(page);
      const localeConfig = await detectLocale(page);
      return { page, disconnect: () => browser.close(), localeConfig };
    } catch (err) {
      if (err instanceof BlockingModalError) throw err;
      last = err;
      if (Date.now() >= deadline) break;
      await new Promise((r) => setTimeout(r, 500));
    }
  }
  throw new Error(
    `No greentap daemon reachable at ${rawUrl} within 30s: ${last?.message}. ` +
      "That daemon owns the WhatsApp session; this process cannot start one.",
  );
}

/**
 * Stop the daemon by sending SIGTERM to its PID.
 * Returns true if daemon was stopped, false if none was running.
 */
export function stopDaemon() {
  const pid = readPid();
  if (!pid) return false;

  if (!isProcessAlive(pid)) {
    cleanupStaleFiles();
    return false;
  }

  process.kill(pid, "SIGTERM");
  return true;
}

/**
 * Get daemon status info.
 * Returns { running, pid, port } or { running: false }.
 */
export function daemonStatus() {
  const pid = readPid();
  const port = readPort();

  if (!pid || !port) return { running: false };

  if (!isProcessAlive(pid)) {
    cleanupStaleFiles();
    return { running: false };
  }

  return { running: true, pid, port };
}
