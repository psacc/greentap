...
/**
 * Capture an ARIA snapshot containing only message rows that intersect the
 * message panel's scroll viewport. WhatsApp keeps older rows rendered above
 * the viewport as a DOM buffer; Playwright's `:visible` selector still matches
 * them, so geometry is required to preserve the default `read` contract.
 *
 * @param {import("playwright").Page} page
 * @returns {Promise<{aria: string, linkRows: Array<{links: Array<{href: string, text: string}>, rowText: string}>}>}
 */
export async function captureVisibleMessageSnapshot(page) {
  const rows = page.locator('#main [role="row"]');
  const inspectRows = () => rows.evaluateAll((elements) => {
    const rowKey = Symbol.for("greentap.message-row-key");
    const sequenceKey = Symbol.for("greentap.message-row-sequence");
    return elements
      .filter((row) => !row.closest('[role="grid"]'))
      .map((row) => {
        if (!row[rowKey]) {
          globalThis[sequenceKey] = (globalThis[sequenceKey] || 0) + 1;
          row[rowKey] = String(globalThis[sequenceKey]);
        }
        ...
        return {
          ...
          visible:
            ...
            rowRect.width > 0 &&
            rowRect.height > 0 &&
            rowRect.bottom > clipRect.top &&
            rowRect.top < clipRect.bottom &&
            rowRect.right > clipRect.left &&
            rowRect.left < clipRect.right,
        };
      });
  });

  // ARIA capture and DOM geometry are separate browser operations. Verify the
  // exact same row nodes on both sides of the snapshot so a same-count rerender
  // cannot apply a positional visibility mask to different messages.
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const before = await inspectRows();
    const aria = await page.locator(":root").ariaSnapshot();
    const after = await inspectRows();
    const stable =
      before.length === after.length &&
      before.every(
        (row, index) =>
          row.key === after[index].key &&
          row.visible === after[index].visible &&
          row.rowText === after[index].rowText &&
          JSON.stringify(row.links) === JSON.stringify(after[index].links),
      );
    const hasViewport = before.every((row) => row.visible !== null);
    if (stable && hasViewport) {
      const filtered = filterMessageRowsByVisibility(aria, before);
      if (filtered !== null) {
        return {
          aria: filtered,
          linkRows: before
            .filter((row) => row.visible && row.links.length > 0)
            .map((row) => ({ links: row.links, rowText: row.rowText })),
        };
      }
    }
  ...
  throw new Error(
    "Could not capture a stable message viewport; WhatsApp rows changed during the ARIA snapshot.",
  );
}

/**
 * Backward-compatible ARIA-only facade used by focused callers and tests.
 * Default read uses captureVisibleMessageSnapshot so parser rows and link
 * enrichment come from the same stable visible-row transaction.
 *
 * @param {import("playwright").Page} page
 * @returns {Promise<string>}
 */
export async function captureVisibleMessageAria(page) {
  return (await captureVisibleMessageSnapshot(page)).aria;
}
 ...
 *   1. A link href is contained in `msg.text`.
 *      — survives differences between the DOM row text and ARIA text.
 *   2. A link's visible text is contained in `msg.text`.
 *   3. `msg.text` non-empty and contained in `linkRow.rowText`
 *      — normal case (text message with a link inline).
 *   4. `msg.text === ""` and `msg.time` contained in `linkRow.rowText`
...
export function mergeLinksIntoMessages(messages, linkRows) {
  ...
  for (const linkRow of linkRows) {
    const findRemainingIndex = (predicate) => {
      for (let scanIdx = msgIdx; scanIdx < out.length; scanIdx++) {
        if (predicate(out[scanIdx])) return scanIdx;
      }
      return -1;
    };

    let matchedIdx = findRemainingIndex((msg) =>
      linkRow.links.some(
        (link) => link.href && msg.text?.includes(link.href),
      ),
    );
    if (matchedIdx < 0) {
      matchedIdx = findRemainingIndex((msg) =>
        linkRow.links.some(
          (link) => link.text && msg.text?.includes(link.text),
        ),
      );
    }
    if (matchedIdx < 0) {
      matchedIdx = findRemainingIndex((msg) =>
        Boolean(msg.text?.length && linkRow.rowText.includes(msg.text)),
      );
    }
    if (matchedIdx < 0) {
      matchedIdx = findRemainingIndex((msg) =>
    ...
    if (matchedIdx >= 0) {
      out[matchedIdx].links = linkRow.links;
    }
    // A DOM link row can correspond to a row the parser omitted. Do not let
    // that unmatched row consume the remaining message cursor; later link
    // rows can still have valid monotone matches.
    if (matchedIdx >= 0) msgIdx = matchedIdx + 1;
...
export async function read(page, chatName, { scroll = false, localeConfig, index, withLinks = true } = {}) {
  ...
  let snapshot = await captureVisibleMessageSnapshot(page);
  let aria = snapshot.aria;
  let messages = parseMessages(aria, { localeConfig, now: readNow });
  // If the first snapshot caught the message panel mid-render (just-opened
  // chat, image-heavy thread, slow network), the parser returns []. Retry
  // ONCE after 800ms before falling through to the no-link path. Avoids
  // the "default read returns [], must add --scroll" UX trap Neko hit
  // on GROUP_Y — without paying the full scrollAndCollect cost.
  if (messages.length === 0) {
    await new Promise((r) => setTimeout(r, 800));
    snapshot = await captureVisibleMessageSnapshot(page);
    aria = snapshot.aria;
    messages = parseMessages(aria, { localeConfig, now: readNow });
  }
  if (!withLinks) return messages;

  return mergeLinksIntoMessages(messages, snapshot.linkRows);
}
...
export async function send(page, chatName, message, localeConfig, index) {
  ...
  // A rendered link preview can add a close button after Send, so target the
  // locale-independent WhatsApp icon instead of relying on button position.
  await humanDelay(200, 400);
  const sendButtons = page
    .getByRole("contentinfo")
    .locator('button:has([data-icon="wds-ic-send-filled"])');
  const sendButtonCount = await sendButtons.count();
  if (sendButtonCount !== 1) {
    throw new Error(
      `Expected one structural Send button after typing; found ${sendButtonCount}.`,
    );
  }
