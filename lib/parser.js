 ...
 * @returns {string[]} Stable image-button blocks in row order.
 */
function extractImageButtonBlocks(rowBody) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (allImg) blocks.push([line.trim(), ...children].join("\n"));
  }
  return blocks;
}
 ...
 *   - don't match clean contact names in downstream summaries
...
/**
 * Remove message-row blocks that are rendered in WhatsApp's DOM buffer but do
 * not intersect the message scroll viewport. Sender buttons and date
 * separators remain in the snapshot so the first visible continuation row
 * keeps its attribution and timestamp context.
 *
 * `visibleRows` follows the DOM order of message rows outside grids. If the
 * DOM and ARIA row counts differ because WhatsApp rerendered between the two
 * reads, return null so the caller can retry rather than return extra or
 * missing messages.
 *
 * @param {string} ariaText
 * @param {Array<{visible: boolean}>} visibleRows
 * @returns {string|null}
 */
export function filterMessageRowsByVisibility(ariaText, visibleRows) {
  if (!ariaText) return ariaText;
  if (!Array.isArray(visibleRows)) return null;

  const lines = ariaText.split("\n");
  const messageRowIndexes = [];

  for (let i = 0; i < lines.length; i++) {
    if (!/^  - (?:')?row "(?:[^"\\]|\\.)*"(?:')?(?::.*)?\s*$/.test(lines[i])) {
      continue;
    }
    ...
    if (!insideGrid) messageRowIndexes.push(i);
  }

  if (messageRowIndexes.length !== visibleRows.length) return null;

  const visibility = visibleRows.map((row) => row?.visible);
  if (visibility.some((visible) => typeof visible !== "boolean")) return null;

  const hiddenStarts = new Set(
    messageRowIndexes.filter((_, index) => !visibility[index]),
  );
  const filtered = [];
  for (let i = 0; i < lines.length; i++) {
    if (!hiddenStarts.has(i)) {
      filtered.push(lines[i]);
      continue;
    }
    while (i + 1 < lines.length && /^ {4}/.test(lines[i + 1])) i += 1;
  }
...
export function parseMessages(ariaText, options = {}) {
  ...
  const messages = [];
  const imageSignatureOccurrences = new Map();
  let currentSender = "";
  ...
  for (let i = msgStartIdx; i < lines.length; i++) {
    ...
    // design-system set (observed: `wds-ic-read` and `wds-ic-delivered`). We
    // match only known delivery receipts from BOTH families; a broad wds-ic-*
    // match would misclassify unrelated WhatsApp icons as own-message proof.
    ...
    const isOwn =
      /img "(?:msg-dblcheck|msg-check|wds-ic-check|wds-ic-dblcheck|wds-ic-read|wds-ic-delivered)"/.test(
        rowBody,
      );
    ...
    const imageButtonBlocks = extractImageButtonBlocks(rowBody);
    if (imageButtonBlocks.length > 0) {
      const timestamp = formatTimestamp(currentDate, time);
      for (const imageBlock of imageButtonBlocks) {
        // Derive identity from the attachment block, not the whole row label:
        // the latter includes a mutable delivery-status word, which made an
        // imageId change between send, delivered, and read states. An
        // occurrence suffix keeps identical same-minute attachments unique
        // within one snapshot while remaining stable across repeated parses.
        const signature = `${sender}|${timestamp ?? time}|${imageBlock}`;
        const occurrence = imageSignatureOccurrences.get(signature) || 0;
        imageSignatureOccurrences.set(signature, occurrence + 1);
        const imageId = simpleHash(`${signature}|${occurrence}`);
        messages.push({
          kind: "image",
          sender,
          text: "",
