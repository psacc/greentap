...
export function parseMessages(ariaText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (timeNode) {
      ...
      // Try extracting from the row label — time with delivery status (own msgs)
      const labelTime = rowLabel.match(/(\d{1,2}:\d{2})\s+(?:Letto|Consegnato)/);
      if (labelTime) {
        time = labelTime[1];
      } else {
        // Time at end of row label (other people's msgs, or end of label before metadata)
    ...
    // Detect own messages: "Tu:" anywhere in label (may be preceded by error text in groups)
    // OR delivery status icon (msg-dblcheck / msg-check)
    const isOwn = /\bTu:/.test(rowLabel) || /img "msg-dblcheck"|img "msg-check"/.test(rowBody);
