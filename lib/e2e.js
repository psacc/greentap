...
async function stageLink(page, localeConfig, sandbox) {
  ...
  // PR #17 ships links[] on each message; assert the marker URL appears in
  // links[].href of some echoed message.
  const hit = messages.find(
    (m) => m.links && m.links.some((l) => l.href && l.href.includes(marker)),
  );
  ...
  return {
    ...
    duration_ms: now() - start,
    messagesInspected: messages.length,
    error: pass ? undefined : `marker '${marker}' not found in any message's links[].href`,
  };
}
...
export async function runE2E({ page, localeConfig }) {
  ...
  // Image stage remains skipped until PR #18 (image download) rebase lands.
  stages.push({
    stage: "image",
    pass: true,
    skipped: true,
  ...
  // Silence unused-symbol lint for stageImage while the image stage is still
  // skipped. (Intentional; tagged with suppression instead of deleting.)
  void stageImage;
  ...
  if (!link.pass) {
    writeLastRun();
    return { pass: false, exitCode: 1, stages };
  }
