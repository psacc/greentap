/**
 * greentap e2e — round-trip verification against the sandbox chat.
 *
 * Invoked via `GREENTAP_E2E=1 node greentap.js e2e`. Runs ordered stages;
 * first failure aborts. Output is structural JSON (counts, paths, bool)
 * — no message content is logged.
 */

import { randomUUID } from "crypto";
import { existsSync, readFileSync, writeFileSync, mkdirSync } from "fs";
import { join, dirname } from "path";
import { homedir } from "os";
import { fileURLToPath } from "url";
import * as commands from "./commands.js";
import { ensureChatList } from "./client.js";
import { isE2EMode } from "./e2e-guard.js";
import { parseMessages } from "./parser.js";

const GREENTAP_DIR = join(homedir(), ".greentap");
const LAST_RUN_FILE = join(GREENTAP_DIR, "e2e-last-run");
const RATE_LIMIT_MS = 60_000;

const __dirname = dirname(fileURLToPath(import.meta.url));
const FIXTURE_PNG = join(__dirname, "..", "test", "fixtures", "e2e", "sample.png");

function now() { return Date.now(); }

/**
 * Exercise client readiness against a browser-blocking informational modal.
 * The fixture mirrors the live modal's structural signature without using
 * translated UI text or reading chat content.
 */
async function stageModal(page) {
  const start = now();
  const injectedSelector = "[data-greentap-e2e-modal]";

  try {
    await ensureChatList(page);
    await page.evaluate(() => {
      delete document.documentElement.dataset.greentapE2eModalAccepted;
      document.querySelector("[data-greentap-e2e-modal]")?.remove();

      const dialog = document.createElement("div");
      dialog.setAttribute("role", "dialog");
      dialog.setAttribute("aria-modal", "true");
      dialog.setAttribute("aria-labelledby", "greentap-e2e-modal-title");
      dialog.setAttribute("data-greentap-e2e-modal", "true");
      Object.assign(dialog.style, {
        position: "fixed",
        inset: "0",
        zIndex: "2147483647",
        background: "white",
      });
      dialog.innerHTML = `
        <div data-testid="confirm-popup" data-animate-modal-body="true">
          <h1 id="greentap-e2e-modal-title" data-testid="popup-title">
            <svg viewBox="0 0 88 88" width="88" height="88"></svg>
            <button type="button"><svg viewBox="0 0 24 24"></svg></button>
          </h1>
          <div data-testid="popup-contents" style="position: absolute; top: 120px; left: 16px">
            <svg viewBox="0 0 24 24"></svg>
            <svg viewBox="0 0 24 24"></svg>
            <svg viewBox="0 0 24 24"></svg>
            <svg viewBox="0 0 24 24"></svg>
            <button
              type="button"
              data-testid="whats_new_continue_button"
              style="width: 48px; height: 24px"
            ></button>
          </div>
        </div>
      `;
      dialog
        .querySelector('[data-testid="popup-contents"] button')
        .addEventListener("click", () => {
          document.documentElement.dataset.greentapE2eModalAccepted = "true";
          dialog.remove();
        });
      document.body.append(dialog);
    });

    await ensureChatList(page);
    const state = await page.evaluate(() => ({
      accepted:
        document.documentElement.dataset.greentapE2eModalAccepted === "true",
      blockingModalPresent: Boolean(
        document.querySelector('[role="dialog"][aria-modal="true"]'),
      ),
    }));
    const pass = state.accepted && !state.blockingModalPresent;
    return {
      stage: "modal",
      pass,
      duration_ms: now() - start,
      error: pass
        ? undefined
        : "known informational modal did not dismiss cleanly",
    };
  } catch (err) {
    return {
      stage: "modal",
      pass: false,
      duration_ms: now() - start,
      error: `modal recovery failed: ${err.message}`,
    };
  } finally {
    await page.evaluate((selector) => {
      document.querySelector(selector)?.remove();
      delete document.documentElement.dataset.greentapE2eModalAccepted;
    }, injectedSelector).catch(() => {});
  }
}

/**
 * Pre-flight: confirm the sandbox group is reachable by navigating to it.
 *
 * Earlier approaches via `chats()` (viewport-bound) or `search()` (its
 * `textbox.first()` falls onto the compose textbox when a chat is
 * already open) were both unreliable. Instead, just call `navigateToChat`
 * directly — its R5 fast-path is a no-op if we're already in the
 * sandbox, otherwise its grid-or-search logic handles cold state. If
 * navigation succeeds, the sandbox exists and is open; if it throws
 * "Chat not found", preflight fails with the same error path.
 *
 * Side effect: stages 2-4 begin with the sandbox chat already open.
 * That's fine — `send`/`read`/`fetchImages` are now re-entrant safe.
 */
async function stagePreflight(page, localeConfig) {
  const start = now();
  const sandbox = process.env.GREENTAP_E2E_CHAT || "greentap-sandbox";
  try {
    await commands.navigateToChat(page, sandbox, localeConfig);
    return {
      stage: "preflight",
      pass: true,
      duration_ms: now() - start,
    };
  } catch (err) {
    return {
      stage: "preflight",
      pass: false,
      duration_ms: now() - start,
      error:
        `sandbox '${sandbox}' not reachable: ${err.message}. ` +
        `Ensure a WhatsApp group named '${sandbox}' exists with only the maintainer as member.`,
    };
  }
}

function checkRateLimit() {
  if (process.env.GREENTAP_E2E_SKIP_RATE_LIMIT === "1") return null;
  if (!existsSync(LAST_RUN_FILE)) return null;
  try {
    const last = parseInt(readFileSync(LAST_RUN_FILE, "utf8").trim(), 10);
    if (!Number.isFinite(last)) return null;
    const ago = now() - last;
    if (ago < RATE_LIMIT_MS) {
      return Math.ceil((RATE_LIMIT_MS - ago) / 1000);
    }
  } catch { /* ignore */ }
  return null;
}

function writeLastRun() {
  mkdirSync(GREENTAP_DIR, { recursive: true, mode: 0o700 });
  writeFileSync(LAST_RUN_FILE, String(now()), { mode: 0o600 });
}

/**
 * Hard-reset the page state. WA's image-preview overlay can linger after
 * setInputFiles/Enter and intercept clicks on the compose box for the
 * next stage. Pressing Escape repeatedly is unreliable — Escape can open
 * unintended menus (verified empirically: 5x Escape from clean state
 * opens 1 dialog). page.reload() is heavier (~3s) but gives a clean,
 * authenticated chat-list state from which navigateToChat works
 * deterministically.
 */
async function resetPageState(page) {
  try {
    await page.reload();
    // Wait for chat list grid to be back. 15s upper bound covers cold
    // WA loads; in steady state it's <2s.
    await page.getByRole("grid").first().waitFor({ timeout: 15000 });
  } catch { /* fall through; the next stage will surface the error */ }
}

/**
 * Attach the fixture image and send.
 *
 * Earlier attempts called setInputFiles on the first
 * `input[type="file"][accept*="image"]` in the page — that turned out
 * to be the STICKER input, not the photo input. The fixture would be
 * sent as an animated sticker rather than a photo.
 *
 * Correct flow:
 *   1. Click "Allega" — the first button in contentinfo's compose toolbar.
 *   2. The attach menu pops up with 8 items: Documento, Foto e video,
 *      Fotocamera, Audio, Contatto, Sondaggio, Evento, Nuovo sticker.
 *      Item 1 (icon `ic-filter-filled`) is "Photos & Videos". Picking
 *      by icon name (visible in the menuitem's textContent) is
 *      locale-agnostic and avoids accidentally selecting Nuovo sticker
 *      (icon `wds-ic-sticker-*`).
 *   3. Clicking that menuitem fires a native filechooser. Playwright
 *      intercepts via `page.waitForEvent("filechooser")`. setFiles on
 *      the chooser populates the photo preview dialog.
 *   4. Click the exact structural media-preview send control. Never use a
 *      page-wide Enter: an unrelated focused dialog could accept it.
 */
async function sendFixtureImage(page, sandbox, fixturePath, localeConfig) {
  await commands.navigateToChat(page, sandbox, localeConfig);

  // Keep a handle to the current tail row. After submitting the preview, a
  // different tail row with image nodes proves that WhatsApp appended media;
  // a fixed delay can return while upload/rendering is still in flight.
  const previousTail = await page.evaluateHandle(() => {
    const rows = [...document.querySelectorAll('#main [role="row"]')]
      .filter((row) => !row.closest('[role="grid"]'));
    return rows.at(-1) ?? null;
  });

  try {
    // Open the attach menu.
    const allegaButton = page
      .getByRole("contentinfo")
      .getByRole("button")
      .first();
    await allegaButton.click({ timeout: 5000 });
    await new Promise((r) => setTimeout(r, 500));

    // Find the "Photos & Videos" menuitem by icon name (locale-agnostic).
    // ic-filter-filled is unique to that item in current WA Web; stickers
    // use wds-ic-sticker-*, document uses ic-description-*, etc.
    // Assert uniqueness — if WA ever adds a second menuitem with the same
    // icon, fail loudly rather than silently picking the first match.
    const photosMatch = await page.evaluate(() => {
      const items = [...document.querySelectorAll('[role="menuitem"]')];
      const indices = items
        .map((m, i) => (/ic-filter-filled/.test(m.textContent || "") ? i : -1))
        .filter((i) => i >= 0);
      return { indices, total: items.length };
    });
    if (photosMatch.indices.length === 0) {
      throw new Error(
        "Photos & Videos menuitem (icon ic-filter-filled) not found in attach menu",
      );
    }
    if (photosMatch.indices.length > 1) {
      throw new Error(
        `ic-filter-filled matched ${photosMatch.indices.length} menuitems (expected 1) — WA UI may have changed; refusing to guess`,
      );
    }
    const photosIdx = photosMatch.indices[0];

    // Set up the filechooser intercept BEFORE clicking — the click fires
    // the event synchronously.
    const fcPromise = page.waitForEvent("filechooser", { timeout: 8000 });
    await page.getByRole("menuitem").nth(photosIdx).click();
    const fileChooser = await fcPromise;
    await fileChooser.setFiles(fixturePath);
    await submitMediaPreview(page);

    // Submission is complete only when a new message row becomes the tail and
    // contains the two structural image nodes (placeholder + blob thumbnail).
    await page.waitForFunction(
      (oldTail) => {
        const rows = [...document.querySelectorAll('#main [role="row"]')]
          .filter((row) => !row.closest('[role="grid"]'));
        const tail = rows.at(-1);
        return Boolean(
          tail &&
          tail !== oldTail &&
          tail.querySelector('img[src^="blob:"]') &&
          tail.querySelectorAll("img").length >= 2
        );
      },
      previousTail,
      { timeout: 15000 },
    );
  } finally {
    await previousTail.dispose();
  }
}

/**
 * Submit only the known WhatsApp media-preview surface. The preview is not an
 * ARIA modal, so require its exact structural markers and its exact send icon.
 * Any visible ARIA modal is unknown in this state and blocks submission.
 *
 * @param {import("playwright").Page} page
 * @param {{timeout?: number}} options
 */
export async function submitMediaPreview(page, { timeout = 10000 } = {}) {
  const canvas = page.locator('[data-testid="media-editor-canvas"]:visible');
  const caption = page.locator(
    '[data-testid="media-caption-input-container"]:visible',
  );
  const sendButton = page.locator(
    '[role="button"]:has([data-testid="wds-ic-send-filled"]):visible',
  );

  await Promise.all([
    canvas.waitFor({ timeout }),
    caption.waitFor({ timeout }),
    sendButton.waitFor({ timeout }),
  ]);
  const [canvasCount, captionCount, sendCount] = await Promise.all([
    canvas.count(),
    caption.count(),
    sendButton.count(),
  ]);
  if (canvasCount !== 1 || captionCount !== 1 || sendCount !== 1) {
    throw new Error(
      "Media preview structure was ambiguous; refusing to submit",
    );
  }

  const unknownDialogs = page.locator(
    '[role="dialog"][aria-modal="true"]:visible',
  );
  if ((await unknownDialogs.count()) !== 0) {
    throw new Error(
      "Unknown blocking dialog appeared over the media preview; refusing to submit",
    );
  }

  await sendButton.click({ timeout });
}

async function stageText(page, localeConfig, sandbox) {
  const start = now();
  const marker = `e2e-probe-${randomUUID()}`;
  const offViewportMarker = `e2e-offviewport-${randomUUID()}`;
  try {
    await commands.send(page, sandbox, marker, localeConfig);
  } catch (err) {
    return { stage: "text", pass: false, duration_ms: now() - start, error: `send failed: ${err.message}` };
  }
  // Delivery rendering is asynchronous. Wait for the exact generated row
  // before read() applies its viewport filter; a fixed delay can snapshot the
  // previous visible set just before the sent row mounts.
  try {
    await page
      .locator('#main [role="row"]')
      .filter({ hasText: marker })
      .waitFor({ timeout: 10000 });
  } catch {
    return {
      stage: "text",
      pass: false,
      duration_ms: now() - start,
      error: "sent text marker did not render within 10s",
    };
  }

  // Inject one clone of the just-sent synthetic row outside the scroll
  // viewport. It is still exposed by the full ARIA tree, exactly like WA's
  // rendered DOM buffer, but default read() must exclude it. This makes the
  // visibility fix an actual E2E assertion: disabling the filter turns the
  // stage red without sending additional messages.
  let probeInRawAria = false;
  const removeOffViewportProbe = async () => {
    const probes = page.locator('[data-greentap-e2e-offviewport]');
    try {
      await probes.evaluateAll((elements) =>
        elements.forEach((element) => element.remove()),
      );
      if ((await probes.count()) !== 0) {
        throw new Error("synthetic off-viewport row remained mounted");
      }
      return null;
    } catch (err) {
      return err;
    }
  };
  try {
    const injected = await page.evaluate(({ sourceMarker, probeMarker }) => {
      document.querySelector('[data-greentap-e2e-offviewport]')?.remove();
      const rows = [...document.querySelectorAll('#main [role="row"]')]
        .filter((row) => !row.closest('[role="grid"]'));
      const source = rows.find((row) =>
        (row.textContent || "").includes(sourceMarker),
      );
      const firstRow = rows[0];
      const sourceWrapper = source?.parentElement;
      const firstWrapper = firstRow?.parentElement;
      if (
        !sourceWrapper?.parentElement ||
        !firstWrapper?.parentElement ||
        sourceWrapper.parentElement !== firstWrapper.parentElement
      ) return false;

      const cloneWrapper = sourceWrapper.cloneNode(true);
      const clone = cloneWrapper.querySelector('[role="row"]');
      if (!clone) return false;
      cloneWrapper.setAttribute("data-greentap-e2e-offviewport", "true");
      const sourceTime = [...(source.textContent || "").matchAll(
        /\b(?:[01]\d|2[0-3]):[0-5]\d\b/g,
      )].at(-1)?.[0] || "00:00";
      clone.setAttribute("aria-label", `${probeMarker} ${sourceTime}`);
      const probeText = document.createElement("span");
      probeText.textContent = probeMarker;
      clone.prepend(probeText);
      const walker = document.createTreeWalker(clone, NodeFilter.SHOW_TEXT);
      let textNode;
      while ((textNode = walker.nextNode())) {
        textNode.nodeValue = (textNode.nodeValue || "").replace(
          sourceMarker,
          probeMarker,
        );
      }
      // Insert the clone as a real earlier row, then keep the viewport pinned
      // to the latest messages. This matches WhatsApp's rendered DOM buffer:
      // the row remains in normal accessibility-tree flow but sits above the
      // scroll viewport. Transform-based positioning was intermittent because
      // Chromium can prune far-transformed nodes from the accessibility tree.
      firstWrapper.parentElement.insertBefore(cloneWrapper, firstWrapper);
      let scroller = cloneWrapper.parentElement;
      while (scroller) {
        const overflowY = getComputedStyle(scroller).overflowY;
        if (overflowY === "auto" || overflowY === "scroll") break;
        scroller = scroller.parentElement;
      }
      if (!scroller) {
        cloneWrapper.remove();
        return false;
      }
      scroller.scrollTop = scroller.scrollHeight;
      return true;
    }, { sourceMarker: marker, probeMarker: offViewportMarker });
    if (!injected) {
      return {
        stage: "text",
        pass: false,
        duration_ms: now() - start,
        error: "could not create the off-viewport message probe",
      };
    }

    const rawAria = await page.locator(":root").ariaSnapshot();
    const rawMessages = (await import("./parser.js")).parseMessages(rawAria, {
      localeConfig,
    });
    probeInRawAria = rawMessages.some((message) =>
      message.text?.includes(offViewportMarker),
    );
    if (!probeInRawAria) {
      const cleanupError = await removeOffViewportProbe();
      return {
        stage: "text",
        pass: false,
        duration_ms: now() - start,
        error: cleanupError
          ? `off-viewport probe cleanup failed: ${cleanupError.message}`
          : "off-viewport probe was not exposed by the raw ARIA tree",
      };
    }
  } catch (err) {
    const cleanupError = await removeOffViewportProbe();
    return {
      stage: "text",
      pass: false,
      duration_ms: now() - start,
      error: cleanupError
        ? `off-viewport probe failed: ${err.message}; cleanup failed: ${cleanupError.message}`
        : `off-viewport probe failed: ${err.message}`,
    };
  }

  let messages;
  let readError = null;
  try {
    messages = await commands.read(page, sandbox, { localeConfig });
  } catch (err) {
    readError = err;
  }
  const cleanupError = await removeOffViewportProbe();
  if (cleanupError) {
    return {
      stage: "text",
      pass: false,
      duration_ms: now() - start,
      error: `off-viewport probe cleanup failed: ${cleanupError.message}`,
    };
  }
  if (readError) {
    return {
      stage: "text",
      pass: false,
      duration_ms: now() - start,
      error: `read failed: ${readError.message}`,
    };
  }
  const hit = messages.find((m) => m.text && m.text.includes(marker));
  const leakedProbe = messages.some((message) =>
    message.text?.includes(offViewportMarker),
  );
  const pass = Boolean(
    probeInRawAria && hit && hit.sender === "You" && !leakedProbe,
  );
  return {
    stage: "text",
    pass,
    duration_ms: now() - start,
    messagesInspected: messages.length,
    offViewportProbeExcluded: probeInRawAria && !leakedProbe,
    error: pass
      ? undefined
      : leakedProbe
        ? "off-viewport message probe leaked into default read"
        : "sent text marker did not round-trip as an own message",
  };
}

/**
 * Correlate the parsed tail image with a newly appended occurrence. Image IDs
 * can repeat when identical structural rows are sent within the same minute,
 * so compare multiplicity rather than set membership.
 *
 * @param {string[]} beforeImageIds
 * @param {Array<{kind: string, imageId?: string}>} afterMessages
 * @returns {string|null}
 */
export function findAppendedTailImageId(beforeImageIds, afterMessages) {
  const tail = afterMessages.at(-1);
  if (tail?.kind !== "image" || !tail.imageId) return null;
  const beforeCount = beforeImageIds.filter((id) => id === tail.imageId).length;
  const afterCount = afterMessages.filter(
    (message) =>
      message.kind === "image" && message.imageId === tail.imageId,
  ).length;
  return afterCount > beforeCount ? tail.imageId : null;
}

/**
 * Capture the pre-send image identity multiset. A missing baseline must reject:
 * treating it as an empty list can let stale media satisfy the E2E stage.
 *
 * @param {import("playwright").Page} page
 * @param {object} localeConfig
 * @returns {Promise<string[]>}
 */
export async function captureImageBaseline(page, localeConfig) {
  const aria = await page.locator(":root").ariaSnapshot();
  const messages = parseMessages(aria, { localeConfig });
  if (messages.length === 0) {
    throw new Error(
      "Pre-send image baseline contained no parseable messages",
    );
  }
  return messages
    .filter((message) => message.kind === "image")
    .map((message) => message.imageId);
}

export async function stageImage(
  page,
  localeConfig,
  sandbox,
  {
    captureBaseline = captureImageBaseline,
    sendImage = sendFixtureImage,
    fetchImages = commands.fetchImages,
  } = {},
) {
  const start = now();
  if (!existsSync(FIXTURE_PNG)) {
    return {
      stage: "image",
      pass: false,
      duration_ms: now() - start,
      error: `fixture missing: ${FIXTURE_PNG}`,
    };
  }

  let beforeImageIds = [];
  try {
    beforeImageIds = await captureBaseline(page, localeConfig);
  } catch (err) {
    return {
      stage: "image",
      pass: false,
      duration_ms: now() - start,
      error: `could not capture pre-send image baseline: ${err.message}`,
    };
  }

  try {
    await sendImage(page, sandbox, FIXTURE_PNG, localeConfig);
  } catch (err) {
    return {
      stage: "image",
      pass: false,
      duration_ms: now() - start,
      error: `attach+send failed: ${err.message}`,
    };
  }

  // The DOM append above proves this is a new row, not stale chat history.
  // ARIA and parser visibility can still lag the DOM briefly, so poll until
  // the newly appended tail is exposed as an image-kind message.
  let newImageId = null;
  const parseDeadline = now() + 10000;
  while (!newImageId && now() < parseDeadline) {
    try {
      const afterAria = await page.locator(":root").ariaSnapshot();
      const afterMsgs = parseMessages(afterAria, { localeConfig });
      newImageId = findAppendedTailImageId(beforeImageIds, afterMsgs);
    } catch { /* retry until the bounded deadline */ }
    if (!newImageId) await new Promise((r) => setTimeout(r, 250));
  }

  if (!newImageId) {
    return {
      stage: "image",
      pass: false,
      duration_ms: now() - start,
      error:
        "new media row did not become an image-kind parser result within 10s",
    };
  }

  let downloads;
  try {
    downloads = await fetchImages(page, sandbox, {
      localeConfig,
      limit: 1,
    });
  } catch (err) {
    return {
      stage: "image",
      pass: false,
      duration_ms: now() - start,
      newImageId,
      error: `fetchImages failed: ${err.message}`,
    };
  }
  // Match the just-sent image by imageId. fetchImages with limit:1
  // returns the most-recent image-kind message, so it should match.
  const top = downloads.find((d) => d.imageId === newImageId) ?? downloads[0];
  const pass = Boolean(
    top && top.path && !top.error && top.imageId === newImageId,
  );
  return {
    stage: "image",
    pass,
    duration_ms: now() - start,
    newImageId,
    imagePath: top?.path ?? null,
    mimeType: top?.mimeType ?? null,
    // Surfaced for the agent caller: after stage passes, the runtime
    // operator (or supervising agent) is expected to Read() the path
    // and confirm the fixture text is legible. CONTRIBUTING.md docs
    // this as the multimodal verification step.
    multimodalCheck: pass
      ? "Read the imagePath and confirm 'GREENTAP-E2E' is legible"
      : null,
    error: pass
      ? undefined
      : top?.error ?? "fetched image does not match newImageId",
  };
}

async function stageLink(page, localeConfig, sandbox) {
  const start = now();
  const marker = randomUUID();
  const url = `https://example.com/e2e-${marker}`;
  try {
    await commands.send(page, sandbox, url, localeConfig);
  } catch (err) {
    return {
      stage: "link",
      pass: false,
      duration_ms: now() - start,
      error: `send failed: ${err.message}`,
    };
  }
  // Link rendering is asynchronous. Wait for the exact generated anchor
  // before asking read() to enrich message rows; a fixed delay races WA's
  // preview pipeline and can inspect the row before its href exists.
  try {
    await page.locator(`a[href*="${marker}"]`).waitFor({ timeout: 10000 });
  } catch {
    return {
      stage: "link",
      pass: false,
      duration_ms: now() - start,
      error: "sent URL did not render as a link within 10s",
    };
  }

  let messages;
  try {
    messages = await commands.read(page, sandbox, {
      localeConfig,
      withLinks: true,
    });
  } catch (err) {
    return {
      stage: "link",
      pass: false,
      duration_ms: now() - start,
      error: `read failed: ${err.message}`,
    };
  }
  const hit = messages.find(
    (m) =>
      m.links && m.links.some((l) => l.href && l.href.includes(marker)),
  );
  const pass = Boolean(hit);
  // Surface the actual matching href, not just links[0] — if a message
  // has multiple links the marker could be at a higher index.
  const matchingHref =
    hit && hit.links.find((l) => l.href.includes(marker))?.href;
  return {
    stage: "link",
    pass,
    duration_ms: now() - start,
    messagesInspected: messages.length,
    hrefFound: matchingHref ?? null,
    error: pass
      ? undefined
      : `marker '${marker}' not found in any message's links[]`,
  };
}

/**
 * Runs the full e2e sequence. Returns { pass, exitCode, stages, rateLimitedForSec? }.
 * exitCode: 0 pass, 1 fail, 2 sandbox missing, 3 rate limited.
 */
export async function runE2E({ page, localeConfig }) {
  if (!isE2EMode()) {
    return {
      pass: false,
      exitCode: 1,
      stages: [{ stage: "init", pass: false, error: "GREENTAP_E2E=1 not set" }],
    };
  }

  const rateSec = checkRateLimit();
  if (rateSec !== null) {
    return { pass: false, exitCode: 3, rateLimitedForSec: rateSec, stages: [] };
  }

  const stages = [];
  const sandbox = process.env.GREENTAP_E2E_CHAT || "greentap-sandbox";

  // Reset page to a known clean state. Previous CLI invocations may
  // have left attachment previews, search overlays, or menus open.
  await resetPageState(page);

  const modal = await stageModal(page);
  stages.push(modal);
  if (!modal.pass) return { pass: false, exitCode: 1, stages };

  const pre = await stagePreflight(page, localeConfig);
  stages.push(pre);
  if (!pre.pass) return { pass: false, exitCode: 2, stages };

  const text = await stageText(page, localeConfig, sandbox);
  stages.push(text);
  if (!text.pass) return { pass: false, exitCode: 1, stages };

  const image = await stageImage(page, localeConfig, sandbox);
  stages.push(image);
  if (!image.pass) return { pass: false, exitCode: 1, stages };

  // After image stage, WA leaves the attachment-preview overlay state
  // partially mounted (verified: Escape opens unintended dialogs from
  // clean state, so we can't dismiss). Reload page → clean compose.
  await resetPageState(page);

  const link = await stageLink(page, localeConfig, sandbox);
  stages.push(link);
  if (!link.pass) return { pass: false, exitCode: 1, stages };

  writeLastRun();
  return { pass: true, exitCode: 0, stages };
}
